...
char	**ft_clean_input(char *input, int file_num)
{
	...
	// printf("NUMBER OF FILES %d AND INPUT = %s\n", file_num, input);
	...
	while (++aux < file_num)
	{
		...
		printf("FILE %s AND NEXT POSITION %d\n", files[aux], i);
	}
	return (files);
}

char	**ft_read_map_files()  // hola.map      que.map tal.map      p
	{
	int		size;
	int		file_num;
	char	buf[1024];
	...
	size = read(1, buf, 1024);
	buf[size - 1] = '\0';
	write(1, "\n", 1);
	// printf("INPUT DE FILES: %s\n", buf);
	file_num = ft_count_files(buf);
	return (ft_clean_input(buf, file_num));
}